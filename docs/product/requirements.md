...
| Roles and permissions | Known requirement | Backend-enforced RBAC protects administrative operations. The current catalog contract uses the accepted five catalog/inventory/media/display-setting permissions with explicit `SUPER_ADMIN` grants; non-Super-Admin Role composition requires a separate owner decision. |
