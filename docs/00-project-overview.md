...
The Backend implements `POST /api/v1/auth/login`, `POST /api/v1/auth/refresh`, `POST /api/v1/auth/logout`, `GET /api/v1/auth/csrf`, `GET /api/v1/auth/me`, protected Category management, bounded protected Product reads, atomic Draft Product/Variant/Inventory creation, Product lifecycle/update, retained Variant update/reactivation, optimistic-version absolute Inventory update, and protected/public singleton rial/toman display-setting routes, plus strict Access-cookie/current-state enforcement, exact catalog permissions, session-bound mutation CSRF, refresh rotation/recovery/reuse handling, current-session logout, and trusted one-shot first-Super-Admin provisioning. The Admin provides a Persian RTL login, memory-only CSRF/session bootstrap, protected-entry gate, minimal authenticated shell, centralized credentialed Axios/error handling, single-flight expired-Access recovery with bounded refresh retry and one-time request replay, and accessible current-session logout. Catalog UI, Product Image operations, and broader public catalog behavior remain later tasks. Generated Swagger/OpenAPI remains available at `/api/docs` only in development and test; production documentation is disabled.
