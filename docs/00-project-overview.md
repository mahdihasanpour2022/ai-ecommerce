...
The Backend implements `POST /api/v1/auth/login`, `POST /api/v1/auth/refresh`, `POST /api/v1/auth/logout`, `GET /api/v1/auth/csrf`, `GET /api/v1/auth/me`, protected Category tree/create/rename/move/eligible-delete routes, strict Access-cookie/current-state enforcement, exact catalog permissions, session-bound mutation CSRF, refresh rotation/recovery/reuse handling, current-session logout, and trusted one-shot first-Super-Admin provisioning. The Admin provides a Persian RTL login, memory-only CSRF/session bootstrap, protected-entry gate, minimal authenticated shell, centralized credentialed Axios/error handling, single-flight expired-Access recovery with bounded refresh retry and one-time request replay, and accessible current-session logout. Catalog UI and Product/Variant behavior described in later plans do not yet exist. Generated Swagger/OpenAPI remains available at `/api/docs` only in development and test; production documentation is disabled.
