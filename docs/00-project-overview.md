...
No Admin login/browser authentication flow, business UI, API business endpoint, session/token runtime behavior, or product behavior described in later plans currently exists. The API reserves `/api/v1` for future REST contracts and exposes generated Swagger/OpenAPI at `/api/docs` only in development and test; production documentation remains disabled. A trusted one-shot command can create only the first eligible Super Admin; it exposes no HTTP route.
