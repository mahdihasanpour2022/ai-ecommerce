...
The Backend implements `POST /api/v1/auth/login`, `GET /api/v1/auth/csrf`, `GET /api/v1/auth/me`, strict Access-cookie/current-state enforcement, minimum `admin.access` RBAC, and trusted one-shot first-Super-Admin provisioning. Refresh rotation, logout, frontend authentication behavior, business UI, and product behavior described in later plans do not yet exist. Generated Swagger/OpenAPI remains available at `/api/docs` only in development and test; production documentation is disabled.
