...
| ----------- | --------------------------------- | ---------: | ----------------------- | ------------------------------------------------------------------------ |
| Storefront  | `@e-commerce/storefront` |       3000 | `http://localhost:3000` | None currently                                                           |
| Admin       | `@e-commerce/admin`      |       3001 | `http://localhost:3001` | Public API base URL below                                                |
| API         | `@e-commerce/api`        |       3002 | `http://localhost:3002` | Runtime/database, authentication, and Product Image storage values below |
...
```text
yarn workspace @e-commerce/storefront dev
...
| `TEST_DATABASE_URL`                 | API persistence integration tests          | PostgreSQL connection URL for `e_commerce_test`                             | Optional outside database integration validation                             | Server-only; credential-bearing                  |
| `PRODUCT_IMAGE_STORAGE_ROOT`        | API Product/Category Image storage         | Non-root filesystem path resolved by the API                                | Required in development/test; disabled and ignored in production             | Server-only, non-secret                          |
...
| `ADMIN_BOOTSTRAP_USERNAME`          | Trusted Admin provisioner                  | Canonical lowercase ASCII username matching `^[a-z0-9_]{3,20}$`             | Required only for the one-shot provisioning process                          | Server-only, identity data                       |
...
| `ADMIN_BOOTSTRAP_PASSWORD`          | Trusted Admin provisioner                  | Exactly six ASCII digits (`^[0-9]{6}$`); no silent normalization/truncation | Required only for the one-shot provisioning process                          | Secret; never tracked, logged, or passed in argv |
...
| `ADMIN_CREDENTIAL_USERNAME`         | Trusted initial-credential updater         | Canonical lowercase ASCII username matching `^[a-z0-9_]{3,20}$`             | Required only for the one-shot initial credential update                     | Server-only, identity data                       |
| `ADMIN_CREDENTIAL_PASSWORD`         | Trusted initial-credential updater         | Exactly six ASCII digits (`^[0-9]{6}$`)                                     | Required only for the one-shot initial credential update                     | Secret; never tracked, logged, or passed in argv |
| `ADMIN_CREDENTIAL_PASSWORD_CONFIRM` | Trusted initial-credential updater         | Exact confirmation                                                          | Required only for the one-shot initial credential update                     | Secret; never tracked, logged, or passed in argv |
...
The `ADMIN_BOOTSTRAP_*` and `ADMIN_CREDENTIAL_*` variables are intentionally absent from `.env.example` values and are not runtime application configuration. Supply them only to the applicable trusted one-shot command through [First Super Admin Provisioning](development/admin-provisioning.md) or [Initial Admin Credential Update](development/admin-credential-update.md); compile before secret injection.

Next.js also assigns its own standard `NODE_ENV`; it is framework-owned rather than an application setting and must not be overridden with a nonstandard value. The Storefront currently consumes no application environment variable. The Admin server consumes the non-public Backend location below.

| Name           | Owner | Type and allowed values                                                | Requirement/default                                  | Exposure                                         |
| -------------- | ----- | ---------------------------------------------------------------------- | ---------------------------------------------------- | ------------------------------------------------ |
| `API_BASE_URL` | Admin | Absolute HTTP(S) REST base URL without credentials, query, or fragment | Optional; defaults to `http://localhost:3002/api/v1` | Server-only BFF/Proxy destination; never bundled |
...
| `AUTH_JWT_ISSUER`                       | Exact string; default `e-commerce-api`                                    | Server-only, non-secret required `iss` value                                                                                                     |
| `AUTH_JWT_AUDIENCE`                     | Exact string; default `e-commerce-admin`                                  | Server-only, non-secret required `aud` value                                                                                                     |
...
`API_BASE_URL` is runtime-owned and does not enter frontend bundles or the build cache key.
...
`PRODUCT_IMAGE_STORAGE_ROOT` contains durable application data rather than disposable build output. The local default `apps/api/tmp/product-images/` is ignored by Git and must never be manually cleaned, renamed, or used by tests. Product/Category image replacement and owner deletion are the only supported removal paths: they commit a durable cleanup record before deleting bytes and retry failed cleanup. Tests provide a separate temporary storage root.
