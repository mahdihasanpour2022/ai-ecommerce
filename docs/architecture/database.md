...
The API owns a Prisma 7.10.0 configuration, generated-client boundary, the approved nine-model Admin authentication schema, and its reviewed additive initial migration. The owner-approved design is recorded in the [S1-T02 schema proposal](../work/sprint-01/s1-t02-schema-proposal.md). S1-T03 implemented and verified those persistence structures against isolated disposable PostgreSQL development/test databases. The canonical generation and migration-review process is defined in [Prisma Development and Migration Workflow](../development/prisma.md).
...
The detailed field types, nullability, constraints, indexes, lifecycle, cleanup, and initial migration plan are canonical in the [S1-T02 schema proposal](../work/sprint-01/s1-t02-schema-proposal.md). `schema.prisma` and the reviewed migration SQL implement that design, and S1-T03 verified the migration and database-enforced invariants on isolated disposable databases. Runtime authentication repositories/services remain later-task work.
