...
The Backend API is a strict-TypeScript NestJS Modular Monolith at `apps/api`. It contains the root application boundary, Admin authentication module, and catalog module with implemented protected Category, Product, Product Variant, and Inventory contracts. Commerce-setting, media, public catalog, order, and customer capabilities are added only by their approved tasks; this architecture is not a directive to create empty modules.
