...
- A reusable same-origin BFF/HTTP/auth layer centralizes Backend routing, typed canonical `ApiResponse` consumption, credential cookies, CSRF headers, timeout, error-code handling, refresh coordination, and observability hooks. The repository-owned Backend guarantees the JSON envelope through global success/error boundaries, so ordinary feature fetchers return typed `response.data` directly without per-feature runtime response parsing. The HTTP boundary places the unchanged safe Backend error-envelope message on standard `AdminHttpError.message`; only failures without a Backend response receive a centralized safe Persian fallback, allowing feature hooks to display `error.message` directly. Low-level authentication/security handling may still validate security-sensitive responses defensively and fail closed. JavaScript does not read either authentication token or construct a Bearer header. The session-bound CSRF token is the only frontend-readable credential; server responses store it in a host-only `SameSite=Strict` cookie, and it is never persisted in Web Storage.
...
The Admin currently exposes `/login`, the protected `/` and `/categories` entries, and the same-origin `/api/v1/**` BFF. Protected Admin pages share a route-group layout that owns the authenticated header, sidebar, and content region; `/login` remains outside that shell. The former `/catalog/**` frontend route tree and its route-specific clients, UI, hooks, and tests have been removed. Backend catalog contracts and authorization remain implemented independently; any future Admin feature route must be planned and approved before implementation.
