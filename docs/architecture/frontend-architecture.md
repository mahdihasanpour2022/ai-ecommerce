...
See [frontend standards](../standards/frontend.md), [authentication](../security/authentication.md), and [Next.js ADR](adr/0004-use-nextjs-for-web-apps.md). The approved [Admin catalog behavior specification](../features/admin-catalog/specification.md) owns Sprint 3 routes, permission-aware UX, forms, conflicts, accessibility, and the bounded UI/test dependency proposal. The protected shell/read-client boundary and Category management are implemented; Product feature workflows remain later tasks.
