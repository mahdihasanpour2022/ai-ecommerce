...
Admin owns thin typed adapters under `hooks/rq_hooks/`. For ordinary feature mutations, feature hooks declaratively provide the HTTP method, safe same-origin BFF path or path builder, body/query-parameter mappers, endpoint DTOs, and invalidation keys; `useRQSender` owns the actual `mutationFn`, HTTP dispatch, fixed mutation policy, response-data extraction, and awaited invalidation. The delete adapter delegates to this same execution boundary. Low-level authentication may retain custom executors for its defensive parsing and distinct request policy. Adapters are not locations for feature semantics; feature folders still own complete query keys, request/response DTOs, cache reconciliation, and user-facing domain outcomes.
...
The existing Axios client remains responsible for timeout, normalized safe failures, CSRF-header injection, cancellation, exact refresh eligibility, single-flight Refresh, and one-time replay. Every normalized `AdminHttpError` has a safe non-empty Persian `message`: HTTP failures preserve the Backend envelope's message unchanged, and failures without a Backend response use a centralized transport/client fallback. TanStack Query receives the final success/failure after that transport policy, and feature hooks display `error.message` without a second message-building helper. Do not add `axios-auth-refresh`, create a second refresh coordinator, or let both Axios and TanStack independently retry authentication recovery.
...
- The read adapter returns typed Axios `response.data` and standard TanStack query state. It does not parse a repository-owned successful response again or own global notification deduplication through mutable module state.
- `useRQSender` returns typed mutation data, constructs ordinary mutation execution from declarative request metadata, and accepts exact feature callbacks/options without weakening the centralized request policy. `useRQDeleter` follows the same boundary.
...
The reusable hooks remain generic over feature DTOs and `AdminHttpError`. The Backend owns and guarantees the canonical `ApiResponse<Result, SingleResult, Details>` envelope for every JSON application response through the globally registered success interceptor and exception filter. Because Backend and frontend are governed in the same repository, ordinary feature hooks type the endpoint response and consume Axios `response.data` directly; they do not duplicate runtime envelope or domain-payload validation.

This decision supersedes the earlier rejection of a global success envelope. The owner approved the replacement on 2026-09-08 and approved direct typed frontend consumption without ordinary feature-level runtime parsing on 2026-09-11. The existing Axios security boundary, `/api/v1/**` BFF, single-flight Refresh, and Query cache ownership remain unchanged. Low-level authentication/security boundaries may retain defensive validation where malformed input must fail closed. Successful image-content routes remain raw binary so browsers can stream and cache them directly; their failures still use `ApiResponse`.

## Storefront continuity

Future Storefront client requests follow the same provider -> feature hook -> thin RQ adapter -> Axios/BFF -> Backend structure, query-key discipline, cache/retry/invalidation rules, direct typed consumption of the Backend-owned JSON envelope, and identity-change cleanup. Code may be shared only after real compatible behavior is demonstrated. Storefront remains public today; future Customer authentication uses separate Customer cookies/sessions/contracts and never Admin cache or credentials.
