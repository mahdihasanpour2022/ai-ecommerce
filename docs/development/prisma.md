...
```text
yarn workspace @automotive-commerce/api prisma:format
yarn workspace @automotive-commerce/api prisma:validate
yarn workspace @automotive-commerce/api prisma:generate
yarn workspace @automotive-commerce/api prisma:migrate:create -- --name <descriptive-name>
yarn workspace @automotive-commerce/api prisma:migrate:deploy
```
...
- `prisma:migrate:create` runs `migrate dev --create-only`. It is an intentional development-only action against `automotive_dev`; it requires a descriptive name and a running local database, creates SQL, and may use a shadow database. It must not be automated against shared or production-like data.
