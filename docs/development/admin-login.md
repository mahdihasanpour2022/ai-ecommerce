...
The Backend implements `POST /api/v1/auth/login`, `POST /api/v1/auth/refresh`, `POST /api/v1/auth/logout`, `GET /api/v1/auth/csrf`, `GET /api/v1/auth/me`, strict Access-cookie/current-state enforcement, and reusable unsafe-request CSRF validation. The Admin frontend implements login, reload bootstrap, protected-entry gating, a minimal protected shell, the centralized Axios transport boundary, single-flight expired-Access recovery, and current-session logout.
...
The authenticated shell exposes a native Persian logout button. It sends no body, requires the current memory CSRF credential, is refresh-ineligible, and coalesces duplicate activation. While pending, the button is disabled and announced busy. Success clears the memory credential and protected state before the existing route gate redirects to login. A transport/server/CSRF failure keeps the current shell and credential available for an explicit retry with accessible Persian feedback; a definitive authentication loss clears state according to the stable authentication policy. The browser never reads or attempts to clear HttpOnly cookies itself.
