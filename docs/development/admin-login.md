...
The Backend implements `POST /api/v1/auth/login`, `POST /api/v1/auth/refresh`, `POST /api/v1/auth/logout`, `GET /api/v1/auth/csrf`, `GET /api/v1/auth/me`, strict Access-cookie/current-state enforcement, and reusable unsafe-request CSRF validation. The Admin frontend implements login, reload bootstrap, protected-entry gating, a minimal protected shell, and the centralized Axios transport boundary. Single-flight refresh orchestration remains S1-T11 work.
