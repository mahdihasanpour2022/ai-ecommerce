export const PRODUCT_IMAGE_MAX_BYTES = 409_600;
...
export class ProductImageSanitizationError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'ProductImageSanitizationError';
  }
}
...
export async function imageSanitizer(file: File): Promise<File> {
  ...
  if (file.size >= PRODUCT_IMAGE_MAX_BYTES) {
    fail('حجم تصویر باید کمتر از ۴۰۰ کیلوبایت باشد.');
  }
...
function hasExpectedContainer(bytes: Uint8Array, mediaType: string): boolean {
  if (mediaType === 'image/png') {
    ...
    return startsWith(bytes, signature) && endsWith(bytes, iend);
  }
...
function fail(message: string): never {
  throw new ProductImageSanitizationError(message);
}
