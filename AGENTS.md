...
- Admin currently retains the thin `apps/admin/hooks/rq_hooks/useRQSender.ts` mutation adapter for authentication. Future feature-specific hooks own endpoint DTOs, runtime response parsing, complete query keys, cache reconciliation, invalidation, and domain-safe outcomes; add read/delete adapters only when an approved route needs them. Storefront later mirrors this separation in its own application boundary rather than importing Admin credentials or feature behavior.
