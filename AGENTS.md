...
This repository is a production-oriented clothing-commerce monorepo with three independent applications: a public Storefront, an Admin Panel, and a shared Backend API. The engineering foundation, Admin authentication, and Backend catalog persistence/contracts are established. The former Admin catalog frontend routes have been removed; new Admin feature routes require owner-selected, route-by-route planning and approval. Workspaces and runtime identifiers use the approved `e-commerce` naming, with SQL-safe PostgreSQL identifiers using `e_commerce`.
...
**Mandatory test-scope rule:** after every change, run only the test files and individual scenarios that directly exercise the changed behavior or a dependency demonstrably affected by it. Do not run an entire application, workspace, or repository test suite for an isolated change. A broader suite is allowed only when the change affects shared foundations broadly, the authoritative acceptance criteria explicitly require that suite, or targeted evidence cannot cover a concrete regression risk; record the specific reason before running it. This restriction applies to tests, while typecheck, lint, build, security, contract, and other Definition-of-Done gates must still be scoped to the affected application or files and remain satisfied where applicable.
...
- Number inputs must not display browser-provided increment/decrement spinner arrows in Admin or Storefront. Own this consistently in each application's global stylesheet while preserving the input's numeric semantics, keyboard behavior, validation attributes, accessible name, and mobile input mode.
- Form submit buttons must remain operable while client fields are empty or invalid so submission reveals each field's linked inline validation error. During asynchronous submission or its service mutation, disable the button with a clearly gray appearance, set an accurate busy state, and render a visible spinner inside the button; preserve single-flight submission and accessible button text throughout.
...
- Admin's thin `apps/admin/hooks/rq_hooks/` adapters own TanStack Query execution and ordinary HTTP dispatch. Feature mutation hooks declaratively provide the method, safe same-origin BFF path or path builder, body/query-parameter mappers, endpoint DTOs, complete query keys, cache reconciliation, invalidation, and domain-safe outcomes; they do not repeat `mutationFn` or call `httpClient` directly. Low-level authentication/security mutations may retain a custom executor where defensive response parsing or distinct policy is required. The repository-owned Backend is the authoritative runtime producer of the canonical JSON `ApiResponse` envelope through its global success interceptor and exception filter, so ordinary feature fetchers consume typed `response.data` directly without per-feature runtime response parsing. Raw binary success routes remain explicit exceptions. Storefront later mirrors this separation in its own application boundary rather than importing Admin credentials or feature behavior.
...
- Preserve the existing Axios -> same-origin `/api/v1/**` BFF -> Backend path, CSRF policy, normalized failures, single-flight Refresh, and one-time replay. Ordinary Backend requests marked refresh-eligible enter the shared Refresh flow only for `401 ACCESS_TOKEN_EXPIRED` or `401 AUTHENTICATION_REQUIRED`; other `401` codes remain non-refreshable. After one successful shared Refresh, every affected failed request is replayed exactly once. Login, bootstrap, logout, and the Refresh request itself remain refresh-ineligible, a replayed request must never trigger another Refresh, and a failed or definitive Refresh must settle all waiters without loops or repeated request replay. Every normalized `AdminHttpError` exposes a non-empty safe Persian `message`: an HTTP failure uses the Backend envelope's `message` unchanged, while failures with no Backend response use one centralized transport/client fallback. Feature hooks display `error.message` directly and do not maintain a separate response-message field or feature-level hard-coded error map. Browser Refresh calls remain visible in Network; do not add `axios-auth-refresh`, a duplicate `/api/proxy`, or a second refresh/retry authority.
...
The owner-approved Admin direction fixes `#ed980e` as the strong Admin accent and `#f2b249` as its lighter supporting accent. Use dark foreground text on these amber fills where white text would not meet contrast, and retain neutral white/gray surfaces as the dominant visual language. Select and review a new representative page before propagating a visual pattern broadly.
